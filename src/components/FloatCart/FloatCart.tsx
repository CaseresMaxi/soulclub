...
export const FloatCart: React.FC<any> = (props): any => {
  ...
  const createMenssaje = (product: any, talle: any) => {
    ...
    window.open(
      `https://wa.me/+5491158277219?text=Articulo:%20${product.name}%0DPrecio:%20${product.price}%0DTalle:%20${talle}`,
      "_blank"
    );
  };
  return (
    <div className="floating-cart-container">
      <div>
        <img alt="" className="floating-cart-img" src={props.product.src}></img>
      </div>
      <div className="name-container">{props.product.name}</div>
      {props.product.talles.map((t: string) => (
        <button
          onClick={() => {
            setTalle(t);
          }}
          className={talle === t ? "talle-btn-selected" : "talle-btn"}
        >
          {t}
        </button>
      ))}
      ...
      <button
        ...
        onClick={() => createMenssaje(props.product, talle)}
      >
        <img
          alt=""
          className="whatsapp-logo"
          src={talle !== "" ? WhatsappLogo : WhatsappLogoGrey}
        ></img>
      </button>
    </div>
  );
